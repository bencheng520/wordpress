...
if ( function_exists('register_nav_menus') ) {
	register_nav_menus(array('primary' => '导航栏'));
}
...
?>
	<li <?php comment_class(); ?> <?php if( $depth > 1){echo ' style="margin-left:' . ceil(80/$depth) . 'px;"';} ?> id="li-comment-<?php comment_ID() ?>" >
		<div id="comment-<?php comment_ID(); ?>" class="comment-body">
			...
			<div class="vcard"><?php printf(__('%s'), get_comment_author_link()) ?>
			
		<font style="color:#DDD;">于&nbsp;&nbsp;<?php printf(__('%1$s - %2$s'), get_comment_date(),  get_comment_time()) ?>说：</font>
...
global $user_ID;
$isSpam = 0;
if ( strtolower(trim($incoming_comment['comment_author'])) == '公子' ) $isSpam = 1;
if ( strtolower(trim($incoming_comment['comment_author_email'])) == 'xxxx@gongzi.org') $isSpam = 1;
    ...
    if(has_post_thumbnail()){ 
       the_post_thumbnail();
        ...
        if(empty($post_img_src)){ 
			$random = mt_rand(1, 10);   
			$post_img = bloginfo('template_url').  '/images/random/img'.$random.'.jpg';
        }
        echo $post_img;
    }
}
